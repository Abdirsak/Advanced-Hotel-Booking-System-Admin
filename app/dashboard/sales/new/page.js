...
//   const [invoiceNo,setInvoiceNo] = useState(invoicesData?.data?.docs[invoicesData.data.docs.length -1].invoiceNo)
...
const SalesFormRegistration = ({ showModal, setShowModal, selectedRow, setSelectedRow }) => {
  ...
  },[formData.customer,formData.totalAmount,formData.salesItems])
  const [errors, setErrors] = useState({});
  const { data: customersData } = useCustomers();
  const { data: productsData } = useProducts();
  const { data: invoicesData } = useInvoices();
  ...
  const invoiceNo =invoicesData?.data?.docs[invoicesData.data.docs.length -1].invoiceNo + 1
  ...
  const handleSubmit = (e) => {
    ...
    //   mutate(formData);
    // }
    const updatedFormData = { ...formData, invoiceNo };
  ...
  return (
    <Fragment>
       ...
       <InvoiceTemplate ref={invoiceTemplateRef} invoiceData={{
         ...
         invoiceNo,
         customerTel: "", // Add the customer's telephone if available in the customer data
